<script>
    ...
    import { BsIcon } from '$lib/components'
...
<div class="d-flex flex-row justify-content-between align-items-center mb-2">
    <div class="actions gap-1">
        ...
        <span class="action-item cursor-pointer" on:click={() => searchEnabled = enableAction()}>
            <BsIcon name="search" size=12 class={searchEnabled && 'text-primary'} />
        </span>
        <div
            class="action-item"
        ...
        >
            {#if searchEnabled}
            <input out:scale={{delay:500}} type="text" bind:value={search} class="form-control form-control-sm" />
            {/if}
        </div>
        
        <!-- EXPORT -->
        <!-- <span class="action-item cursor-pointer" on:click={() => exportEnabled = enableAction()}>
            <BsIcon name="file-earmark-arrow-down" size=12 />
        </span>
        <div
            class="action-item"
        ...
        <span class="action-item cursor-pointer" on:click={() => recordsEnabled = enableAction()}>
            <BsIcon name="list-ol" size=12 />
        </span>
        <div
            class="action-item"
            style={`width:${$widthRecords}px`}
        >
            {#if recordsEnabled}
            <select out:scale={{delay:500}} bind:value={recordsPerPage} type="text" class="form-select form-select-sm">
                <option selected="true" disabled="disabled">Items per page</option>
                <option>5</option>
                <option>10</option>
                <option>25</option>
                <option>50</option>
